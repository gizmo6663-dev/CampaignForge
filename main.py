...
try:
    ...
    def ensure_dirs():
        ...
        for d in [IMG_DIR, MUSIC_DIR, MAPS_DIR]:
            try:
...
''')
    ...
    class CampaignForgeApp(App):
        ...
        # ---------- LYD (Musikk + Ambient) ----------
        def _mk_lyd(self):
            """Lyd-fane med sub-tabs: Musikk og Ambient."""
            ...
            b_mus = RToggle(
                text='Musikk', group='lyd_sub',
                state='down' if self._lyd_sub == 'mus' else 'normal',
                bg_color=BTNH if self._lyd_sub == 'mus' else BTN,
                color=GOLD if self._lyd_sub == 'mus' else DIM,
                font_size=sp(11), bold=True)
            b_mus.bind(on_release=lambda b: self._lyd_switch('mus'))
            sub_bar.add_widget(b_mus)

            b_amb = RToggle(
                text='Ambient', group='lyd_sub',
                state='down' if self._lyd_sub == 'amb' else 'normal',
                bg_color=BTNH if self._lyd_sub == 'amb' else BTN,
                color=GOLD if self._lyd_sub == 'amb' else DIM,
                font_size=sp(11), bold=True)
            b_amb.bind(on_release=lambda b: self._lyd_switch('amb'))
        ...
        def _lyd_switch(self, which):
            """Bytt mellom musikk- og ambient-visning."""
            self._lyd_sub = which
            self._lyd_render_sub()
